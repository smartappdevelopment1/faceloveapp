...
import 'package:hookup4u/Screens/Calling/dial.dart';
...
class _ChatPageState extends State<ChatPage> {
            ...
            PopupMenuButton(itemBuilder: (ct) {
              return [
                PopupMenuItem(
                  ...
                  value: 1,
                  child: InkWell(
                    child: Text(isBlocked ? "$unblockUserText" : "$blockUserText"),
                    onTap: () {
                      Navigator.pop(ct);
                      showDialog(
                        context: context,
                        builder: (BuildContext ctx) {
                          return AlertDialog(
                            title: Text(isBlocked ? '$unblockText' : '$blockText'),
                            content: Text(
                                '$doYouWantToText ${isBlocked ? '$unblockText' : '$blockText'} ${widget.second.name}?'),
                            actions: <Widget>[
                              FlatButton(
                                onPressed: () =>
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Container(
                    ...
                    child: isBlocked
                        ? Text("$sorryYouCantSendMessageText")
                        : _buildTextComposer(),
                  ),
                ],
              ),
            ...
            children: <Widget>[
              Container(
                ...
                child: GestureDetector(
                    ...
                    onTap: () async {
                      toast("$doubleTapForCamera",duration: Duration(microseconds: 300));
  ...
  Future<void> onJoin(callType) async {
    if (!isBlocked) {
      ...
      CustomSnackbar.snackbar("$blockedText", _scaffoldKey);
    }
  }
}
